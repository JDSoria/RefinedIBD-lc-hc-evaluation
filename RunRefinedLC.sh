#!/bin/bash
#SBATCH --job-name=IBD_LC
#SBATCH --partition=mono
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=4
#SBATCH --array=1-22
#SBATCH --output=logLC/IBD_%A_%a.out
#SBATCH --error=logLC/IBD_%A_%a.err

set -euo pipefail

CHR=${SLURM_ARRAY_TASK_ID:-${1:?Usage: bash RunRefinedLC.sh <chr>}}

BASE=${BASE:-/home/dsoria/PoblAR/LocalAncestryMuestrasPiloto/Script/Input}
VCF=${VCF:-/home/dsoria/PoblAR/RefinedIBD/LC_clean_AF01/chr${CHR}_clean.vcf.gz}
MAP=${MAP:-${BASE}/MapasGeneticos/chr${CHR}.b38.gmap.plink}
OUTDIR=${OUTDIR:-/home/dsoria/PoblAR/RefinedIBD/LC}
IMAGE=${IMAGE:-${BASE}/RefinedIBD/refined_ibd.sif}

mkdir -p "${OUTDIR}" logLC
...
echo "Running RefinedIBD LC chromosome ${CHR}"
...
echo "RefinedIBD chr${CHR} DONE (LC)"
...
echo "Merging segments chr${CHR} (LC)"
